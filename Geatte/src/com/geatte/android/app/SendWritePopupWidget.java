...
public class SendWritePopupWidget extends PopupWindow {
    ...
    /**
     * Creates a new SendWritePopupWidget for the given context.
     * 
     * @param context The context in which the SendWritePopupWidget is running in
     */
    public SendWritePopupWidget(Context context) {
	super(context);
	...
	setContentView(LayoutInflater.from(mContext).inflate(R.layout.send_write_view, null));
    ...
    /**
     * Call that method to display the {@link SendWritePopupWidget} anchored to the
     * given view.
     * 
     * @param anchor The view the {@link SendWritePopupWidget} will be anchored to.
     */
    public void show(View anchor) {
	if(Config.LOG_DEBUG_ENABLED) {
	    Log.d(Config.LOGTAG, " SendWritePopupWidget show() START");
	}
	...
	Button btnCancel = (Button)contentView.findViewById(R.id.send_write_cancel_btn);
	...
	final EditText sendTitleEditText = (EditText) contentView.findViewById(R.id.send_edittext_title);
	String savedCapt = prefs.getString(Config.PREF_SEND_CAPTION, null);
	if (savedCapt != null) {
	    sendTitleEditText.setText(savedCapt);
	}
	final EditText sendDescEditText = (EditText) contentView.findViewById(R.id.send_edittext_desc);
	String savedDesc = prefs.getString(Config.PREF_SEND_DESC, null);
	if (savedDesc != null) {
	    sendDescEditText.setText(savedDesc);
	}
	...
	Button btnOk = (Button)contentView.findViewById(R.id.send_write_ok_btn);
	btnOk.setOnClickListener(new OnClickListener() {
	    @Override
	    public void onClick(View vv) {
		String caption = sendTitleEditText.getText().toString();
		if (caption != null && !caption.trim().equals("")) {
		    if(Config.LOG_DEBUG_ENABLED) {
			Log.d(Config.LOGTAG, " SendWritePopupWidget return from caption typing, caption = " + caption);
		    }

		    SharedPreferences.Editor editor = prefs.edit();
		    editor.putString(Config.PREF_SEND_CAPTION, caption);

		    String desc = sendDescEditText.getText().toString();
		    if (desc != null && !desc.trim().equals("")) {
			if(Config.LOG_DEBUG_ENABLED) {
			    Log.d(Config.LOGTAG, " SendWritePopupWidget return from desc typing, desc = " + desc);
			}
	...
	if(Config.LOG_DEBUG_ENABLED) {
	    Log.d(Config.LOGTAG, " SendWritePopupWidget show() END");
	}
    }
