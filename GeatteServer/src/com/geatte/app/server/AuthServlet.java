...
public class AuthServlet extends HttpServlet {
	...
	// If login/logout is complete, redirect to the extension page.
	// Otherwise, send user to
	// login/logout, setting the continue page back to this servlet (since
	// UserService does
	// not understand chrome-extension:// URLs)
	if (req.getParameter("completed") != null) {
	    // Server-side redirects don't work for chrome-extension:// URLs so
	    // we do a client-
	    // side redirect instead

	    // Sanitize the extRet URL for XSS protection
	    String regExChromeOld = "chrome-extension://[a-z]+" + (signIn ? "/signed_in\\.html" : "/signed_out\\.html"); // TODO:
	    // Deprecated
	    // 04/08/10
	    String regExChrome = "chrome-extension://[a-z]+"
		+ (signIn ? "/help\\.html(#signed_in)?" : "/signed_out\\.html");
	    String regExFirefox = "http://code\\.google\\.com/p/chrometophone/logo\\?" + (signIn ? "login" : "logout");
	    if (extRet.matches(regExChromeOld) || extRet.matches(regExChrome) || extRet.matches(regExFirefox)) {
	    ...
	    resp.sendRedirect(signIn ? userService.createLoginURL(followOnURL) : userService
		    .createLogoutURL(followOnURL));
	}
    }
}
